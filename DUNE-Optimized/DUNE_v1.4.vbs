...
Const SongVolume = 0.06 ' 1 is full volume. Value is from 0 to 1
...
Const BallRollVolume = 0.7		  ' Level of ball rolling volume. Value between 0 and 1
...
Dim prev_LFA, prev_RFA, prev_RFA1, prev_Spinner2

Sub RealTime_Timer
    RollingUpdate
    
    Dim curr_LFA, curr_RFA, curr_RFA1, curr_Sp2
    curr_LFA = LeftFlipper.CurrentAngle
    curr_RFA = RightFlipper.CurrentAngle
    curr_RFA1 = RightFlipper1.CurrentAngle
    curr_Sp2 = Spinner2.CurrentAngle
    
    If curr_LFA <> prev_LFA Then : LeftFlipperTop.RotZ = curr_LFA : prev_LFA = curr_LFA : End If
    If curr_RFA <> prev_RFA Then : RightFlipperTop.RotZ = curr_RFA : prev_RFA = curr_RFA : End If
    If curr_RFA1 <> prev_RFA1 Then : RightFlipperTop1.RotZ = curr_RFA1 : prev_RFA1 = curr_RFA1 : End If
    If curr_Sp2 <> prev_Spinner2 Then : FEYD.Roty = curr_Sp2 : prev_Spinner2 = curr_Sp2 : End If
...
Dim SSF_Gain
SSF_Gain = 0.5
SlingshotSoundLevel = 0.95 * SSF_Gain					  'volume multiplier
BumperSoundFactor = 4.25 * SSF_Gain						'volume multiplier
...
Sub RollingUpdate()
	...
	For b = UBound(gBOT) + 1 To tnob - 1
		...
		If AmbientBallShadowOn = 0 Then
			If BallShadowA(b).visible <> 0 Then BallShadowA(b).visible = 0
		End If
		rolling(b) = False
	...
	For b = 0 To UBound(gBOT)
		...
		' Ball Drop Sounds
		If bVelZ <  - 1 And bZ < 55 And bZ > 27 Then 'height adjust for ball drop sounds
		...
		If AmbientBallShadowOn = 0 Then
			...
			If BallShadowA(b).visible <> 1 Then BallShadowA(b).visible = 1
		End If
	Next
End Sub
...
Sub DynamicBSUpdate
	...
	For s = UBound(gBOT) + 1 To tnob - 1
		If objrtx1(s).visible <> 0 Then objrtx1(s).visible = 0
		If objrtx2(s).visible <> 0 Then objrtx2(s).visible = 0
		If objBallShadow(s).visible <> 0 Then objBallShadow(s).visible = 0
		If BallShadowA(s).visible <> 0 Then BallShadowA(s).visible = 0
	Next
	...
	For s = lob To UBound(gBOT)
		Dim bX, bY, bZ
		bZ = gBOT(s).Z : bX = gBOT(s).X : bY = gBOT(s).Y
		Dim bShadowSize
		bShadowSize = (bZ + BallSize) / 80
		...
		If AmbientBallShadowOn = 1 Then
			...
			If bZ > 30 Then
				...
				If Not bsRampType = bsRamp Then 'Primitive visible on PF
					If objBallShadow(s).visible <> 1 Then objBallShadow(s).visible = 1
					objBallShadow(s).X = bX + (bX - (tablewidth / 2)) / (Ballsize / AmbientMovement) + offsetX
					objBallShadow(s).Y = bY + offsetY
					objBallShadow(s).size_x = 5 * bShadowSize 'Shadow gets larger and more diffuse as it moves up
					objBallShadow(s).size_y = 4.5 * bShadowSize
				...
				If bsRampType = bsRampClear Or bsRampType = bsRamp Then 'Flasher visible on opaque ramp
					If BallShadowA(s).visible <> 1 Then BallShadowA(s).visible = 1
					BallShadowA(s).X = bX + offsetX
					BallShadowA(s).Y = bY + offsetY + BallSize / 10
					BallShadowA(s).height = bZ - BallSize / 4 + s / 1000 'This is technically 1/4 of the ball "above" the ramp, but it keeps it from clipping the ramp
			...
			ElseIf bZ <= 30 And bZ > 20 Then
				...
				objBallShadow(s).X = bX + (bX - (tablewidth / 2)) / (Ballsize / AmbientMovement) + offsetX
				objBallShadow(s).Y = bY + offsetY
				'   objBallShadow(s).Z = bZ + s/1000 + 0.04		'Uncomment (and adjust If/Elseif height logic) if you want the primitive shadow on an upper/split pf
			...
			Else
				...
				BallShadowA(s).height = bZ - BallSize / 4 + s / 1000
			End If
		...
		ElseIf AmbientBallShadowOn = 2 Then
			...
			ElseIf bZ <= 30 And bZ > 20 Then 'On pf
				If BallShadowA(s).visible <> 1 Then BallShadowA(s).visible = 1
				BallShadowA(s).X = bX + (bX - (tablewidth / 2)) / (Ballsize / AmbientMovement) + offsetX
				BallShadowA(s).Y = bY + offsetY
		...
		If DynamicBallShadowsOn Then
			If bZ < 30 And bX < 850 Then 'Parameters for where the shadows can show, here they are not visible above the table (no upper pf) or in the plunger lane
				dist1 = falloff
				dist2 = falloff
				For iii = 0 To numberofsources - 1 'Search the 2 nearest influencing lights
					LSd = Distance(bX, bY, DSSources(iii)(0), DSSources(iii)(1)) 'Calculating the Linear distance to the Source
				...
				If dist1 < falloff Then
					If objrtx1(s).visible <> 1 Then objrtx1(s).visible = 1
					objrtx1(s).X = bX
					objrtx1(s).Y = bY
					'   objrtx1(s).Z = bZ - 25 + s/1000 + 0.01 'Uncomment if you want to add shadows to an upper/lower pf
				...
				If dist2 < falloff Then
					If objrtx2(s).visible <> 1 Then objrtx2(s).visible = 1
					objrtx2(s).X = bX
					objrtx2(s).Y = bY + offsetY
					'   objrtx2(s).Z = bZ - 25 + s/1000 + 0.02 'Uncomment if you want to add shadows to an upper/lower pf
